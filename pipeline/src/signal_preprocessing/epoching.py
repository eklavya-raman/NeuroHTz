...
<<<<<<< HEAD
    """Create epochs from STI 014 events, or fall back to fixed-length events."""
    try:
        events = mne.find_events(raw, stim_channel="STI 014")
    except ValueError:
        events = mne.make_fixed_length_events(raw, id=1, duration=max(0.5, tmax - tmin))

    if len(events) == 0:
        events = mne.make_fixed_length_events(raw, id=1, duration=max(0.5, tmax - tmin))

    if "stimulus" in event_id and 1 not in event_id.values():
        event_id = {"stimulus": 1}
